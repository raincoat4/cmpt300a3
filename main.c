...
void* my_malloc(size_t size){
    ...
    if(!jobs_root){
        ...
        a=a+(int)size;
        jobs_root=insert(jobs_root,largeBlockSize-size, a);
        
        return largeBlock;
    }
    struct treeNode* ret = bestFit(jobs_root, size);
    if(!ret){
        ...
        jobs_root=insert(jobs_root, (ret->size)-size, b);
