...
void* my_malloc(size_t size){
    ...
    if(!ret){
        ...
        struct treeNode* usedNode = findNode(free_root, size, a);
        struct treeNode* freeNode = findNode(free_root, (ret->size)-size,b);
        insertMap(startMap, a, usedNode);
        insertMap(endMap, b, freeNode);
        return a;
    }
}
